...
" GUI设置
set guioptions-=T "关闭菜单栏
set guioptions-=m "关闭工具栏
...
"折叠
" set foldenable  "启用折叠                                  
" set foldmethod=indent  "indent设置折叠,或者使用marker折叠方式
" set foldlevel=3 "设置折叠层级
...
"设置字典文件的路径,用于OmniComplete
" set dictionary+=~\tags  "将tags文件作为字典
...
set ft=php " 查看html文件里的javascript
"  < Tagbar >
...
let g:fuf_coveragefile_prompt = '=>' "美化Fuzzyfinder
...
\])|(tmp|log|db/migrate|vendor|**/*.png)' "不查找这些文件
" let g:fuf_enumeratingLimit = 5000  "查找最大数为5000，否则打开fuzzyfinder会报错(貌似不会报错了)
